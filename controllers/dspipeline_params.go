...
func (p *DSPAParams) LoadMlmdCertificates(ctx context.Context, client client.Client, logger logr.Logger) error {
	secret, err := util.GetMlmdGrpcCertificatesSecret(ctx, client, p.Namespace, p.Name)
	if err != nil {
		ignoreSecretNotFound := config.GetBoolConfigWithDefault(config.MlmdIgnoreAutoGeneratedSecretNotFoundError, config.DefaultMlmdIgnoreAutoGeneratedSecretNotFoundError)
	...
	p.MLMD.GRPC.CertificateContents = strings.ReplaceAll(string(secret.Data["tls.crt"]), "\n", "\\n")
	p.MLMD.GRPC.PrivateKeyContents = strings.ReplaceAll(string(secret.Data["tls.key"]), "\n", "\\n")
	return nil
}
